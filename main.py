...
def creer_pokemon_sauvage(niveau_moyen: int) -> Pokemon:
    ...
    if type_choisi == "Feu":
        return PokemonFeu(nom, niveau, base_pv, base_atk)
    elif type_choisi == "Eau":
        return PokemonEau(nom, niveau, base_pv, base_atk)
    else:
        return PokemonPlante(nom, niveau, base_pv, base_atk)
...
def combat_sauvage(joueur: Dresseur):
    ...
    print(f"C'est un {pokemon_sauvage.nom} de Niveau {pokemon_sauvage.niveau} ({pokemon_sauvage.type}).")
    ...
    while not poke_actif.est_ko() and not pokemon_sauvage.est_ko():
        print(f"\n--- Tour {tour} (Sauvage) ---")
        print(f"Ton Pokémon: {poke_actif}")
        print(f"Sauvage : {pokemon_sauvage}")
        ...
        if action == "1":
            ...
            if pokemon_sauvage.est_ko():
                ...
                if nouvelle_evolution != poke_actif:
                    joueur.gerer_evolution(poke_actif, nouvelle_evolution)
            ...
            if poke_actif.est_ko():
                print(f" Ton Pokémon, {poke_actif.nom}, est K.O. ! Combat terminé.")
                break 

        elif action == "2":
            chance_capture = (pokemon_sauvage.pv_max - pokemon_sauvage.points_de_vie) / pokemon_sauvage.pv_max
            chance_capture = max(0.1, chance_capture)
            ...
            print("Tu as réussi à fuir. Le combat est terminé.")
            break
        else:
            print("Action invalide. Choisis 1, 2 ou 3.")
...
def initialiser_jeu() -> Dresseur:
    ...
    base_niv = 10 
    base_pv = 50 + base_niv*3
    base_atk = 15 + base_niv
...
def selection_equipe_arene(joueur: Dresseur):
    ...
    while len(equipe_choisie) < 3:
        ...
        try:
            ...
            print("Entrée invalide. Entre le numéro du Pokémon.")
...
def lancer_aventure():
    ...
    for i, arene in enumerate(arenes):
        ...
        while True:
            ...
            if action == "2":
                ...
                if random.random() < 0.8:
                    combat_sauvage(joueur)
                else:
                    print("Rien ne se passe... continuons la balade.")
                print("\n" + joueur.__str__())
            
            elif action == "1":
                if peut_entrer:
                    print("\n" + "~" * 60)
                    print(f"DÉFI : Arène {arene.type_arene}")
                    print("~" * 60)
                    break 
                else:
                    print("Tu dois avoir au moins 3 Pokémon vivants pour défier l'Arène. Entraîne-toi davantage !")
        ...
        if len(joueur.equipe_combat) < 3:
            print("Défaite avant le combat d'Arène, car l'équipe de combat ne contient pas 3 Pokémon vivants.")
            break
    ...
    if joueur.victoires == len(arenes):
        print("\n" + "!"*50)
        print("!!! Félicitations, tu as obtenu les trois Badges et es prêt pour la Ligue !!!")
        print("!"*50)
