...
class Pokemon(ABC):
    def __init__(self, nom, type_pokemon, niveau, pv_max, attaque_base):
        self.nom = nom
    ...
    def calculer_xp_necessaire(self) -> int: return self.niveau * 10 + 50

    def monter_niveau(self) -> 'Pokemon':
        self.niveau += 1
        print(f"\n✨ {self.nom} atteint le Niveau {self.niveau} !")
        
        pv_gain, atk_gain = random.randint(4, 6), random.randint(1, 3)
        self.pv_max += pv_gain
    ...
    def gagner_xp(self, xp_gagne) -> 'Pokemon':
        if self.niveau >= 100: return self
        ...
        while self.experience >= self.xp_pour_prochain_niveau:
            ...
            if nouvelle_evolution != self: return nouvelle_evolution 
                
        return nouvelle_evolution

    def evoluer(self, nouvelle_nom, nouveau_type_classe) -> 'Pokemon':
        """Effectue l'évolution du Pokémon."""
        print(f"\n🎉 Félicitations ! Votre {self.nom} est en train d'évoluer...")
        
        nouveau_pokemon = nouveau_type_classe(
            nouvelle_nom, self.niveau, self.pv_max + 10, self.attaque_base + 5 
        )
    ...
    def est_ko(self) -> bool: return self.points_de_vie <= 0
        
    @abstractmethod
    def calculer_degats_speciaux(self, autre_pokemon: 'Pokemon') -> float: pass
    
    def attaquer(self, autre_pokemon: 'Pokemon'):
        if self.est_ko(): return
        ...
        message_type = ""
        if multiplicateur > 1.0: message_type = " (Super efficace ! )"
        elif multiplicateur < 1.0: message_type = " (Pas très efficace... )"
...
class PokemonEvoFeu(Pokemon):
    def __init__(self, nom, niveau, pv_max, attaque_base):
...
# Classes de base (peuvent évoluer)
class PokemonFeu(Pokemon):
    def __init__(self, nom, niveau, pv_max, attaque_base):
        super().__init__(nom, "Feu", niveau, pv_max, attaque_base)
        self.evolution_niveau = 15; self.prochain_nom = "Pyroli"; self.prochaine_classe = PokemonEvoFeu
    def calculer_degats_speciaux(self, autre_pokemon: Pokemon) -> float:
        if autre_pokemon.type == "Plante": return 2.0
        elif autre_pokemon.type == "Eau": return 0.5
        return 1.0
...
class PokemonEau(Pokemon):
    def __init__(self, nom, niveau, pv_max, attaque_base):
        super().__init__(nom, "Eau", niveau, pv_max, attaque_base)
        self.evolution_niveau = 15; self.prochain_nom = "Aquali"; self.prochaine_classe = PokemonEvoEau 
    def calculer_degats_speciaux(self, autre_pokemon: Pokemon) -> float:
        if autre_pokemon.type == "Feu": return 2.0
        elif autre_pokemon.type == "Plante": return 0.5
        return 1.0
...
class PokemonPlante(Pokemon):
    def __init__(self, nom, niveau, pv_max, attaque_base):
        super().__init__(nom, "Plante", niveau, pv_max, attaque_base)
        self.evolution_niveau = 15; self.prochain_nom = "Phyllali"; self.prochaine_classe = PokemonEvoPlante 
    def calculer_degats_speciaux(self, autre_pokemon: Pokemon) -> float:
        if autre_pokemon.type == "Eau": return 2.0
        elif autre_pokemon.type == "Feu": return 0.5
        return 1.0